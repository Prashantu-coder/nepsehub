...
const AnalyticsService = {
    ...
    _calculateSentiment(data) {
        ...
        data.forEach(s => {
            const diff = parseFloat(s.percentageChange);
            if (diff > 0) adv++;
            else if (diff < 0) dec++;
            else unc++;
        });
    ...
    _calculateMovers(data) {
        const items = [...data];
        return {
            gainers: [...items].sort((a, b) => parseFloat(b.percentageChange) - parseFloat(a.percentageChange)).slice(0, 5),
            losers:  [...items].sort((a, b) => parseFloat(a.percentageChange) - parseFloat(b.percentageChange)).slice(0, 5),
            active:  [...items].sort((a, b) => (parseFloat(b.totalTradeQuantity) || 0) - (parseFloat(a.totalTradeQuantity) || 0)).slice(0, 5)
        };
    },
    ...
    _calculateSectors(data) {
        ...
        data.forEach(s => {
            ...
            sectorMap[s.sector].changeSum += parseFloat(s.percentageChange) || 0;
